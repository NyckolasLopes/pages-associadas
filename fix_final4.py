...
def fix_checkout(c):
    ...
    c = c.replace('to={`/${currentLoja.slug}/finalizar`}', 'to={`/${(currentLoja as any).slug}/finalizar` as any}')
    return c
modify_file('src/routes/_store.checkout.tsx', fix_checkout)

# 2. _store.perfil.tsx
def fix_perfil(c):
    c = c.replace('supabase.functions.invoke("delete_own_account")', '(supabase.functions.invoke as any)("delete_own_account")')
    return c
modify_file('src/routes/_store.perfil.tsx', fix_perfil)

# 3. _store/p/$slug.tsx
def fix_slug(c):
    c = c.replace('(p.marca || "") ===', '((p as any).marca || "") ===')
    c = c.replace('p.marca ===', '((p as any).marca || "") ===')
...
def fix_carts(c):
    c = c.replace('user={user as any}', 'user={cart?.user as any}')
    c = c.replace('cartItems={cartItems as any}', 'cartItems={cart?.cartItems as any}')
    c = c.replace('user={user}', 'user={cart?.user as any}')
    c = c.replace('cartItems={cartItems}', 'cartItems={cart?.cartItems as any}')
    return c
...
# 8. admin/estoque.tsx
def fix_estoque(c):
    return c.replace('currentLoja as unknown as Loja', 'currentLoja as any')
...
def fix_precos(c):
    c = c.replace("const handleConfirmImportMeusPrecos = () => {", "const handleConfirmImportMeusPrecos = async () => {")
