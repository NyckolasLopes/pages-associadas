...
export const useAdminFiltros = create<FiltroStore>()(
  persist(
    (set, get) => ({
      ...
      loadFiltros: async () => {
        try {
          const raw = await supabaseStorage.getItem("admin-filtros-storage");
          if (raw) {
            const parsed = JSON.parse(raw);
            const stateData = parsed.state || parsed;
            if (stateData.filtros && Array.isArray(stateData.filtros)) {
              set({
                filtros: stateData.filtros,
                storeFiltros: stateData.storeFiltros || get().storeFiltros
              });
            }
          }
        } catch (e) {
          console.warn("Erro ao carregar filtros compartilhados:", e);
        }
      },
