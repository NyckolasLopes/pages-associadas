...
function DynamicVitrines({ local, page = "Página inicial" }: { local: VitrineLocal; page?: string }) {
...
function DynamicTarja({ page = "Página inicial" }: { page?: string }) {
  const allBanners = useAdmin((s) => s.banners || []);
...
function DynamicCategoriaBanners({ page = "Página inicial" }: { page?: string }) {
  const allBanners = useAdmin((s) => s.banners || []);
...
function DynamicExtraBanners({ page = "Página inicial" }: { page?: string }) {
  const allBanners = useAdmin((s) => s.banners || []);
...
function CampaignHighlight() {
  ...
  const campaignProducts = useMemo(() => {
    ...
    const campaignIds = new Set(
      customProducts.filter(cp => isCampanhaAtiva(cp)).map(cp => cp.id)
    );
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 pb-16 md:pb-0 overflow-x-hidden">
        ...
        <section className="container-fa pt-2 pb-6 relative group">
          ...
          <DynamicCategoriaBanners />
        </section>



        <DynamicVitrines local="espaco_1" />
        
        <DynamicExtraBanners />
        
        <ServicesSection />
        
        <DynamicVitrines local="espaco_2" />
        ...
        <DynamicVitrines local="espaco_3" />
