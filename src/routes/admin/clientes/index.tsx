...
const getOrderStatusBadgeClass = (status: string) => {
  ...
  if (s.includes("cancel") || s.includes("recus")) return "bg-red-100 text-red-700 border-red-200";
  if (s.includes("separ")) return "bg-blue-100 text-blue-700 border-blue-200";
  if (s.includes("pront") || s.includes("retirada")) return "bg-orange-100 text-orange-700 border-orange-200";
  if (s.includes("rota")) return "bg-purple-100 text-purple-700 border-purple-200";
  if (s.includes("entreg") || s.includes("concluid") || s.includes("finaliz")) return "bg-teal-100 text-teal-700 border-teal-200";
  if (s.includes("novo") || s.includes("recebid") || s.includes("pendent")) return "bg-sky-100 text-sky-700 border-sky-200";
                          ...
                          <div key={ped.id} className="flex items-center justify-between p-2.5 bg-slate-50 rounded-lg border text-xs">
                            <div>
                              <div className="font-bold text-slate-800 flex items-center gap-1.5">
                                <span>{ped.id}</span>
                                <Badge variant="outline" className={`text-[10px] py-0 px-2 font-bold border ${getOrderStatusBadgeClass(ped.status)}`}>
                                  {ped.status}
                                </Badge>
                              </div>
