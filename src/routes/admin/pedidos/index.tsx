            ...
            <div className="bg-white border shadow-sm rounded-2xl p-6 h-fit space-y-6">
              <div>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-bold text-slate-800 text-lg">Dados do Cliente</h3>
                  {(selectedOrder.cliente.cnpj || selectedOrder.cliente.tipoPessoa === "PJ") ? (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs font-bold text-slate-500 mt-1">
                      {(selectedOrder.cliente.cnpj || selectedOrder.cliente.tipoPessoa === "PJ") ? "Cliente PJ" : "Cliente Loja"}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3 pt-4 border-t border-slate-100">
                {(selectedOrder.cliente.cnpj || selectedOrder.cliente.tipoPessoa === "PJ") ? (
                  <>
                    {selectedOrder.cliente.razaoSocial && (
                      <div className="text-xs text-slate-600">
                        <span className="font-bold text-slate-400 uppercase block text-[10px]">Razão Social</span>
                        <span className="font-medium text-slate-800 text-sm">{selectedOrder.cliente.razaoSocial}</span>
                      </div>
                    )}
                    {selectedOrder.cliente.nomeFantasia && (
                      <div className="text-xs text-slate-600">
                        <span className="font-bold text-slate-400 uppercase block text-[10px]">Nome Fantasia</span>
                        <span className="font-medium text-slate-800 text-sm">{selectedOrder.cliente.nomeFantasia}</span>
                      </div>
                    )}
                    <div className="flex items-center gap-3 text-sm text-slate-600">
                      ...
                      <span className="font-bold text-slate-900">{selectedOrder.cliente.cnpj || "Não informado"}</span>
                    </div>
                    {selectedOrder.cliente.responsavelCompra && (
                      <div className="text-xs text-slate-600">
                        <span className="font-bold text-slate-400 uppercase block text-[10px]">Responsável da Compra</span>
                        <span className="font-medium text-slate-800">{selectedOrder.cliente.responsavelCompra}</span>
                      </div>
                    )}
                    <div className="text-xs text-slate-600">
                      <span className="font-bold text-slate-400 uppercase block text-[10px]">Inscrição Estadual</span>
                      <span className="font-medium text-slate-800">
                        {selectedOrder.cliente.isentoIE || selectedOrder.cliente.inscricaoEstadual === "ISENTO" ? (
                          <Badge className="bg-amber-100 text-amber-800 border-none font-bold text-[10px]">Isento</Badge>
                        ) : (
                          selectedOrder.cliente.inscricaoEstadual || "Não informada"
                        )}
                      </span>
                    </div>
                    {selectedOrder.cliente.informacoesTributarias && (
                      <div className="text-xs text-slate-600">
                        <span className="font-bold text-slate-400 uppercase block text-[10px]">Informações Tributárias</span>
                        <span className="font-medium text-slate-800 bg-slate-100 px-2 py-0.5 rounded inline-block mt-0.5">
                          {selectedOrder.cliente.informacoesTributarias}
                        </span>
                      </div>
                    )}
                ...
                {selectedOrder.cliente.email && (
                  <div className="flex items-center gap-3 text-sm text-slate-600 pt-2 border-t">
